...
UPlist = ['爱做饭的芋头SAMA', '啊吗粽', '阿幕降临', '波桑吃遍世界', '宝剑嫂', '长歌与小见见', 
            ...
            '周六野Zoey', 'zettaranc','木鱼水心', '中国BOY超级大猩猩' ]
for item in UPlist:
    ...
    like   = []
    # 读取数据
    file_path = 'json1/' + item + '.json'
    with open(file_path, 'r', encoding='utf-8') as originFile:
        dataset = json.load(originFile)
        # print(type(data[0]['title']))
        for i in range(len(dataset)):
            title.append(dataset[i]['title'])
            comment.append(dataset[i]['comment'])
            play.append(dataset[i]['play'])
            pic.append(dataset[i]['pic'])
            aid.append(dataset[i]['aid'])
            bvid.append(dataset[i]['bvid'])
            created.append(dataset[i]['created'])
            date.append(dataset[i]['date'])
            time.append(dataset[i]['time'])
            length.append(dataset[i]['length'])
            video_review.append(dataset[i]['video_review'])
            favorite.append(dataset[i]['favorite'])
            coin.append(dataset[i]['coin'])
            share.append(dataset[i]['share'])
            like.append(dataset[i]['like'])
